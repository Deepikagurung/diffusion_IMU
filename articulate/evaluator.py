...
class BasePoseEvaluator:
    ...
    def _preprocess(self, pose, shape=None, tran=None):
        # Ensure pose tensor is on the correct device
        pose = pose.to(self.device)

        # If the evaluator expects rotation matrices but the input
        # appears to be 6D vectors, convert automatically and warn.
        if self.rep == RotationRepresentation.ROTATION_MATRIX:
            # last dim may be joints*9 or joints*6; treat 6 as R6D
            if pose.dim() >= 2 and pose.shape[-1] == 6:
                import warnings
                warnings.warn("Pose input has last-dimension 6 but evaluator is "
                              "configured for ROTATION_MATRIX; treating as R6D "
                              "and converting.")
                pose = to_rotation_matrix(pose, RotationRepresentation.R6D)
            else:
                # verify divisibility by 9 so .view will not break silently
                per_frame = pose.numel() // pose.shape[0]
                if per_frame % 9 != 0:
                    raise ValueError(f"Pose tensor with shape {pose.shape} cannot be "
                                     f"reshaped into rotation matrices. "
                                     f"Expected feature dimension divisible by 9.")
        elif self.rep == RotationRepresentation.R6D:
            # make sure input has the expected 6-dim features
            if pose.dim() >= 2 and pose.shape[-1] != 6:
                raise ValueError(f"Evaluator configured for R6D input but received "
                                 f"tensor with last dim {pose.shape[-1]}.")
        # perform conversion to the internal matrix representation
        pose = to_rotation_matrix(pose, self.rep).view(pose.shape[0], -1)

        # optional sanity check: joint count should match model
        try:
            j, _ = self.model.get_zero_pose_joint_and_vertex(None)
            # j is either [num_joint,3] or [B,num_joint,3]; take second dim if exists
            num_j = j.shape[1] if j.dim() == 3 else j.shape[0]
            actual = pose.numel() // pose.shape[0] // 9
            if actual != num_j:
                raise ValueError(f"Input contains {actual} joints but model has {num_j}. "
                                 "Please check your pose representation or evaluator rep.")
        except Exception:
            # if model query fails, ignore – we only use this for helpful error messages
            pass
