...
class ParametricModel:
    ...
    def view_joint(self, joint_list: list, fps=60, distance_between_subjects=0.8):
        ...
        if verts.shape[0] > 1:
            # Save each rendered sequence to a unique file under `videos/`
            out_dir = 'videos'
            ...
            vo3d.render_sequence_3d(verts, faces, 720, 720, out_path, fps, visible=True)
        else:
    ...
    def view_mesh(self, vertex_list: list, fps=60, colors=None, distance_between_subjects=0.8):
        ...
        if verts.shape[0] > 1:
            # Save each rendered sequence to a unique file under `videos/`
            out_dir = r'C:\deepika\2_mobileposer\MobilePoser\mobileposer\videos'
            os.makedirs(out_dir, exist_ok=True)
            ...
            vo3d.render_sequence_3d(verts, faces, 720, 720, out_path, fps, colors=colors, visible=True)
        else:
            vo3d.vis_mesh(verts[0], faces)
