...
# ============================================================================
# 2. TIME EMBEDDING - Convert timestep t to learnable representation
# ============================================================================
...
# ============================================================================
# 4. DATA PREPARATION - Extract IMU conditions from dataset
# ============================================================================
class DiffusionDataset(Dataset):
    ...
    def __init__(self, fold: str = 'train'):
        ...
        self.pose_dataset = PoseDataset(fold=fold)
...
def train_diffusion_model(epochs: int = 100, 
    ...
    # Load data
    print("\nLoading dataset...")
    dataset = DiffusionDataset(fold='train')
...
# ============================================================================
# 6. INFERENCE/SAMPLING - Generate new poses from noise
# ============================================================================
def sample_from_diffusion(model,
                          cond: torch.Tensor,
                          config = None,
                          num_steps: int = 50,
                          device: str = "cuda") -> torch.Tensor:
    """
    Generate pose+translation samples by reversing the diffusion process.
    
    Process:
    1. Start with random noise
    2. Iteratively denoise using the trained model
    3. Use IMU condition to guide generation
    4. Return final denoised pose+translation
    
    Args:
        model: trained diffusion model
        cond: IMU condition [batch, seq_len, 60]
        num_steps: how many denoising steps (less = faster but lower quality)
    
    Returns:
        generated pose+translation [batch, seq_len, 147]
    """
    if config is None:
        config = DiffusionConfig()
    ...
    x = torch.randn(batch_size, seq_len, 147, device=device)
    x_flat = x.reshape(-1, 147)
    cond_flat = cond.reshape(-1, 60)
    
    # Reverse diffusion: progressively denoise
    with torch.no_grad():
        # Create reverse timestep schedule (1000 → 0 in num_steps)
        timesteps = torch.linspace(config.timesteps - 1, 0, num_steps, device=device).long()
        
        for i, t_idx in enumerate(timesteps):
            t = torch.full((x_flat.shape[0],), t_idx, device=device, dtype=torch.long)
            
            # Predict noise
            noise_pred = model(x_flat, t, cond_flat)
            
            # Compute variance schedule
            beta_t = betas[t].unsqueeze(-1)
            alpha_t = alphas_cumprod[t].unsqueeze(-1)
            alpha_prev_t = alphas_cumprod[t - 1].unsqueeze(-1) if t_idx > 0 else torch.ones_like(alpha_t)
            
            # Denoise step: remove predicted noise
            x_flat = (x_flat - beta_t * noise_pred / torch.sqrt(1 - alpha_t)) / torch.sqrt(alpha_t / alpha_prev_t)
            
            # Add small noise for next step (except final step)
            if i < len(timesteps) - 1:
                x_flat += torch.randn_like(x_flat) * torch.sqrt(beta_t * (1 - alpha_prev_t) / (1 - alpha_t))
    
    x = x_flat.reshape(batch_size, seq_len, 147)
    return x
