...
def training(train_loader, val_loader, model, diffusion, num_epochs=1, device=None, patience: int = 10, min_delta: float = 1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        if best_val_loss is None or val_loss < best_val_loss - min_delta:
            ...
            epochs_no_improve = 0
            torch.save(ema.shadow.state_dict(), "diffusion_model_best.pth")
        else:
            epochs_no_improve += 1
    ...
    torch.save(ema.shadow.state_dict(), "diffusion_model_final.pth")
...
def test_diffusion_model(
    checkpoint_path: str = "diffusion_model_final.pth",
    noise_level: int = 50,
    num_vis_samples: int = 2,
    visualize: bool = True,
):
    """Load a trained diffusion model, denoise validation data, compute MPJPE, and optionally visualize.

    Args:
        checkpoint_path: Path to the saved model weights (.pth).
        noise_level: Diffusion timestep to noise GT to before denoising back (0–999).
                     Lower = less noise = easier reconstruction.
    ...
    for i, batch in enumerate(tqdm(val_loader, desc="Evaluating")):
        ...
        # --- forward noise at `noise_level`, then denoise back via DDIM ---
        t = torch.full((B,), noise_level, device=device, dtype=torch.long)
        noise = torch.randn_like(x0)
        x_noisy, _ = diffusion.q_sample(x0, t, noise)

        with torch.no_grad():
            # Use DDIM denoising from noise_level back to 0
            # Build sub-schedule only for [0, noise_level]
            ...
            for idx, t_cur in enumerate(timesteps):
                ...
                noise_pred = model(x_denoised, t_tensor, cond=imu_cond)
        ...
        for b in range(B):
            ...
            # global -> local (evaluator expects local rotations)
            gt_local   = bodymodel.inverse_kinematics_R(gt_rot)
            pred_local = bodymodel.inverse_kinematics_R(pred_rot)

            error = mpjpe_evaluator(
                pred_local.view(L, -1), gt_local.view(L, -1)
            )
            # error[0] = position error (m), [1] = local angle °, [2] = global angle °
            mpjpe_cm = error[0].item() * 100  # metres -> cm
            all_mpjpe.append(mpjpe_cm)
        ...
        if visualize and i == 0:
            ...
            for b in range(min(num_vis_samples, B)):
                L = int(lengths[b])
                ...
                viewer.view(pred_local, tran_pred, gt_local, tran_gt, with_tran=False)
...
if __name__ == "__main__":
    ...
    if mode == "test":
        # Usage:  python model_diffusion_new.py test [checkpoint] [noise_level]
        ckpt = sys.argv[2] if len(sys.argv) > 2 else "diffusion_model_final.pth"
        nlevel = int(sys.argv[3]) if len(sys.argv) > 3 else 50
        test_diffusion_model(checkpoint_path=ckpt, noise_level=nlevel)
