# Latent Diffusion Model for Pose Estimation — V2 (Separate Decoders)
...
#   - Separate pose decoder + translation decoder
#   - Independently normalized losses with translation upweighting
# Stage 2: Freeze autoencoder, train diffusion in latent space
#   - IMU-conditioned denoiser (actually uses IMU input)
...
# ===========================================================================
# Stage 1: Transformer VAE (Autoencoder) — V2 with separate decoders
# ===========================================================================
...
class PoseTranslationVAE(nn.Module):
    """VAE with separate encoders AND separate decoders for pose/translation.

    Pose encoder:  (B, T, 144) → (B, T, pose_latent_dim)
    Tran encoder:  (B, T, 3)   → (B, T, tran_latent_dim)
    Combined latent: (B, T, pose_latent_dim + tran_latent_dim)
    Pose decoder:  (B, T, latent_dim) → (B, T, 144)
    Tran decoder:  (B, T, latent_dim) → (B, T, 3)
    """
    def __init__(
        self,
        pose_dim: int = 144,
        ...
        self.tran_encoder = TemporalTransformerEncoder(
            ...
            d_model=d_model // 2, nhead=max(1, nhead // 2),
            num_layers=max(2, enc_layers // 2),
            dim_feedforward=dim_feedforward // 2, dropout=dropout,
        )

        # ---- Separate DECODERS ----
        # Pose decoder: full-size transformer
        self.pose_decoder = TemporalTransformerDecoder(
            latent_dim=self.latent_dim, output_dim=pose_dim,
            d_model=d_model, nhead=nhead, num_layers=dec_layers,
            dim_feedforward=dim_feedforward, dropout=dropout,
        )
        # Tran decoder: dedicated smaller transformer for translation
        self.tran_decoder = TemporalTransformerDecoder(
            latent_dim=self.latent_dim, output_dim=tran_dim,
            d_model=d_model // 2, nhead=max(1, nhead // 2),
            num_layers=max(2, dec_layers // 2),
            dim_feedforward=dim_feedforward // 2, dropout=dropout,
        )
    ...
    def decode(self, z):
        """Decode latent to pose + tran via separate decoders."""
        pose_recon = self.pose_decoder(z)
        tran_recon = self.tran_decoder(z)
...
# ===========================================================================
# Stage 1 Training: Autoencoder — V2 with separate losses
# ===========================================================================

def train_autoencoder(
    train_loader, val_loader, vae, num_epochs=50, device=None,
    patience=10, min_delta=1e-4, kl_weight=1e-4, tran_weight=10.0,
):
    """Train the PoseTranslationVAE with independently normalized losses.

    Loss = pose_MSE + tran_weight * tran_MSE + kl_weight * KL
    pose_MSE and tran_MSE are each normalized by their own element count.
    """
    if device is None:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            pose_recon_err = nn.functional.mse_loss(pose_recon, pose, reduction="none")
            tran_recon_err = nn.functional.mse_loss(tran_recon, tran, reduction="none")

            pose_mask = mask.unsqueeze(-1).expand_as(pose_recon_err).float()
            tran_mask = mask.unsqueeze(-1).expand_as(tran_recon_err).float()

            # Independently normalized losses
            pose_loss = (pose_recon_err * pose_mask).sum() / pose_mask.sum()
            tran_loss = (tran_recon_err * tran_mask).sum() / tran_mask.sum()

            # KL divergence
            kl_loss = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())

            loss = pose_loss + tran_weight * tran_loss + kl_weight * kl_loss
            ...
            epoch_pose_loss += pose_loss.item() * num_valid
            epoch_tran_loss += tran_loss.item() * num_valid
            epoch_kl_loss += kl_loss.item() * num_valid
            epoch_total_loss += loss.item() * num_valid
            total_frames += num_valid

        scheduler.step()

        avg_pose = epoch_pose_loss / total_frames if total_frames > 0 else 0.0
        avg_tran = epoch_tran_loss / total_frames if total_frames > 0 else 0.0
        avg_kl = epoch_kl_loss / total_frames if total_frames > 0 else 0.0
        avg_total = epoch_total_loss / total_frames if total_frames > 0 else 0.0

        # Validation
        val_total, val_pose, val_tran, val_kl = validate_autoencoder(
            val_loader, vae, device, kl_weight, tran_weight,
        )

        current_lr = optimizer.param_groups[0]['lr']
        print(
            f"[AE] Epoch {epoch:3d} | "
            f"Train total={avg_total:.6f} pose={avg_pose:.6f} tran={avg_tran:.6f} kl={avg_kl:.6f} | "
            f"Val total={val_total:.6f} pose={val_pose:.6f} tran={val_tran:.6f} | "
            f"LR: {current_lr:.2e}"
        )

        if best_val_loss is None or val_total < best_val_loss - min_delta:
            best_val_loss = val_total
            epochs_no_improve = 0
            torch.save(vae.state_dict(), "vae_best_v2.pth")
        else:
            epochs_no_improve += 1
    ...
    print(f"\n[AE] Final Train Loss: total={avg_total:.6f} pose={avg_pose:.6f} tran={avg_tran:.6f}")
    print(f"[AE] Final Val Loss:   total={val_total:.6f} pose={val_pose:.6f} tran={val_tran:.6f}")
    with open("vae_loss_v2.txt", "w") as f:
        f.write(f"[AE] Final Train: total={avg_total:.6f} pose={avg_pose:.6f} tran={avg_tran:.6f} kl={avg_kl:.6f}\n")
        f.write(f"[AE] Final Val:   total={val_total:.6f} pose={val_pose:.6f} tran={val_tran:.6f} kl={val_kl:.6f}\n")
        f.write(f"[AE] Best Val Loss: {best_val_loss:.6f}\n")
        f.write(f"[AE] tran_weight={tran_weight}, kl_weight={kl_weight}\n")
    torch.save(vae.state_dict(), "vae_final_v2.pth")
    print("Autoencoder training complete.")


def validate_autoencoder(val_loader, vae, device, kl_weight=1e-4, tran_weight=10.0):
    """Returns (total_loss, pose_loss, tran_loss, kl_loss)."""
    vae.eval()
    val_pose_sum = 0.0
    val_tran_sum = 0.0
    val_kl_sum = 0.0
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            pose_loss = (nn.functional.mse_loss(pose_recon, pose, reduction="none") * pose_mask).sum() / pose_mask.sum()
            tran_loss = (nn.functional.mse_loss(tran_recon, tran, reduction="none") * tran_mask).sum() / tran_mask.sum()
            kl_loss = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
            total_loss = pose_loss + tran_weight * tran_loss + kl_weight * kl_loss

            num_valid = mask.sum().item()
            val_pose_sum += pose_loss.item() * num_valid
            val_tran_sum += tran_loss.item() * num_valid
            val_kl_sum += kl_loss.item() * num_valid
            val_total_sum += total_loss.item() * num_valid
            val_frames += num_valid

    if val_frames == 0:
        return 0.0, 0.0, 0.0, 0.0
    return (
        val_total_sum / val_frames,
        val_pose_sum / val_frames,
        val_tran_sum / val_frames,
        val_kl_sum / val_frames,
    )


# ===========================================================================
# Stage 2 Training: Latent Diffusion — V2 with IMU conditioning
# ===========================================================================
...
def train_latent_diffusion(
    ...
    """Train the latent denoiser with frozen VAE, conditioned on IMU."""
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    for epoch in range(num_epochs):
        ...
        if best_val_loss is None or val_loss < best_val_loss - min_delta:
            ...
            epochs_no_improve = 0
            torch.save(ema.shadow.state_dict(), "latent_denoiser_best_v2.pth")
        else:
            epochs_no_improve += 1
...
def generate_samples(vae, denoiser, diffusion, num_samples, seq_len, device, cond=None):
    """Sample latents via DDIM, then decode to pose + translation."""
    vae.eval()
    denoiser.eval()
...
def test_latent_diffusion(
    vae_checkpoint: str = "vae_best_v2.pth",
    denoiser_checkpoint: str = "latent_denoiser_best_v2.pth",
    ...
    for NOISE_LEVEL in noise_levels:
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(val_loader, desc=f"Eval (noise={NOISE_LEVEL})")):
                ...
                z0, _, _ = vae.encode(gt_pose_r6d, gt_tran)
                ...
                # Decode latent back to data space
                pred_pose_r6d, pred_tran = vae.decode(z_denoised)
    ...
    # Save results
    output_path = "latent_diffusion_results_v2.txt"
...
# ===========================================================================
# Visualize: fast video rendering for multiple noise levels (no MPJPE)
# ===========================================================================
...
def visualize_noise_levels(
    ...
    """Quickly visualize predicted vs GT poses at each noise level (no MPJPE eval)."""
    if noise_levels is None:
        noise_levels = [50, 80, 100]
    ...
    diffusion = GaussianDiffusion(num_timesteps=NUM_DIFFUSION_STEPS, schedule="cosine").to(device)
    denoiser = LatentDenoiser(latent_dim=LATENT_DIM, imu_dim=60).to(device)
    denoiser.load_state_dict(torch.load(denoiser_checkpoint, map_location=device, weights_only=False))
    denoiser.eval()
    ...
    # Load just one batch from validation set
    datamodule = PoseDataModule(finetune=None)
    datamodule.setup(stage="fit")
    val_loader = datamodule.val_dataloader()
    ...
    for NOISE_LEVEL in noise_levels:
        print(f"\n--- Visualizing noise level t={NOISE_LEVEL} ---")

        z0, _, _ = vae.encode(gt_pose_r6d, gt_tran)

        t_tensor_full = torch.full((gt_pose_r6d.shape[0],), NOISE_LEVEL, device=device, dtype=torch.long)
        noise = torch.randn_like(z0)
        z_noisy, _ = diffusion.q_sample(z0, t_tensor_full, noise)
        ...
        z_denoised = z_noisy
        for idx, t_cur in enumerate(timesteps):
            t_t = torch.full((gt_pose_r6d.shape[0],), t_cur, device=device, dtype=torch.long)
            noise_pred = denoiser(z_denoised, t_t, cond=None)
        ...
        for b in range(min(num_vis_samples, gt_pose_r6d.shape[0])):
            ...
            gt_local[:, 0] = flip_rot @ gt_local[:, 0]
            pred_local[:, 0] = flip_rot @ pred_local[:, 0]
            print(f"  Sample {b+1}/{min(num_vis_samples, gt_pose_r6d.shape[0])} (noise={NOISE_LEVEL}, len={L})")
            viewer.view(pred_local, pred_tran[b, :L], gt_local, gt_tran[b, :L], with_tran=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", nargs="?", default="train",
                        help="'train_ae', 'train_diffusion', 'train' (both), 'test', or 'visualize'")
    parser.add_argument("--device", type=str, default=None)
    parser.add_argument("--vae-checkpoint", type=str, default="vae_best_v2.pth")
    parser.add_argument("--denoiser-checkpoint", type=str, default="latent_denoiser_best_v2.pth")
    ...
    parser.add_argument("--tran-weight", type=float, default=10.0,
                        help="Weight for translation loss relative to pose loss")
    ...
    if args.mode == "visualize":
        ...
        if args.mode in ("train_ae", "train"):
            print("\n" + "="*60)
            print("STAGE 1: Training Autoencoder (VAE) — V2 separate decoders")
            print("="*60)
            ...
            print(f"Translation loss weight: {args.tran_weight}")

            train_autoencoder(
                train_loader, val_loader, vae,
                num_epochs=args.ae_epochs, device=device,
                patience=10, kl_weight=1e-4, tran_weight=args.tran_weight,
            )

        # ----- Stage 2: Latent Diffusion -----
        if args.mode in ("train_diffusion", "train"):
            print("\n" + "="*60)
            print("STAGE 2: Training Latent Diffusion — V2 with IMU conditioning")
            print("="*60)
